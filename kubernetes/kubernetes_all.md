...
### [FYI only] k8s user data script [Taken from school of devops tut]
...
root@kube-01:~#
```
Adding watch to make it real time and monitor new additions:
```
...
__To detail the list of tokens associated with that master__
```
...
On Master Node or any node were you want to operate kubectl from (can also be your localhost)
...
### To check the cluster info (Cluster health status)
```
...
__To get the version__
```
kubectl version
```
__To get component status__
```
...
root@kube-01:~#
```
__To get pod details with kubectl.__
```
...
```
__Getting to know the events in a cluster__
```
...
__Kubectl describe that svc with the below command mandatory to provide the namespace as this is not avaialable in the default namespace.:__
...
__Wide option provide more details__
```
...
Shorter and better watch command version
```
watch -n 1 kubectl get pods,deploy,rs,svc
```

__Output below for the above watch command (Most efficient and clearer output):__
```
Every 1.0s: kubectl get pods,deploy,rs,svc                                 Wed May 30 10:51:56 2018

NAME                                       READY     STATUS    RESTARTS   AGE
pod/kube-ops-view-6f9c4d8999-9q57x         1/1       Running   0          52m
pod/kube-ops-view-redis-5cb9899f5f-gj9kj   1/1       Running   0          52m

NAME                                        DESIRED   CURRENT   UP-TO-DATE   AVAILABLE   AGE
deployment.extensions/kube-ops-view         1         1         1            1           52m
deployment.extensions/kube-ops-view-redis   1         1         1            1           52m

NAME                                                   DESIRED   CURRENT   READY     AGE
replicaset.extensions/kube-ops-view-6f9c4d8999         1         1         1         52m
replicaset.extensions/kube-ops-view-redis-5cb9899f5f   1         1         1         52m

NAME                          TYPE        CLUSTER-IP       EXTERNAL-IP   PORT(S)        AGE
service/kube-ops-view         NodePort    10.97.124.173    <none>        80:31238/TCP   52m
service/kube-ops-view-redis   ClusterIP   10.100.247.145   <none>        6379/TCP       52m
service/kubernetes            ClusterIP   10.96.0.1        <none>        443/TCP        1d
```

#### Kubectl command to describe a service in k8s: 
